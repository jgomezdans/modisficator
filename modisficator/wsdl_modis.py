...
def wsdl_get_data_tseries ( lon, lat, product, layer, \
    ...
    WSDLFile = 'http://daac.ornl.gov/cgi-bin/MODIS/GLBVIZ_1_Glb_subset/MODIS_webservice.wsdl?WSDL'

    while True:
        try:
            client = Client( WSDLFile, cache=None ) #For some reason, I can't cache...
            break
        except:
    ...
    for year in xrange(year_start, year_end+1):
        for day_start in numpy.arange(1,365,80):
            ...
            ret = client.service.getsubset (lat, lon, product, layer,\
                    "A%4d%03d"%(year,day_start),"A%4d%03d"%(year,day_end),x_pixels, y_pixels)
            ...
            for time_step in xrange(len(ret.subset.item)):
                ...
                date.append( pylab.date2num( datetime.date( time.strptime(\
                    tempo,"A%Y%j")[0],time.strptime(tempo,"A%Y%j")[1],time.strptime(tempo,"A%Y%j")[2])) )
                # Extract QA vals...
                if layer=="sur_refl_day_of_year" or layer=="sur_refl_state_500m":
                    vals = numpy.array ( [int(f) for f in ret.subset.item[time_step].strip().split(",")[5:]])
                    value.append ( vals)
                else:
                    vals = numpy.array ( [float(f) for f in ret.subset.item[time_step].strip().split(",")[5:]])
    ...
    for time_step in xrange(len(ret.subset.item)):
        ...
        if ret.scale == 0.:
            try:
                ...
                vals = numpy.array ( [int(f) for f in response.strip().split(",")[5:]])
            value.append ( vals)
        else:
            try:
                vals = numpy.array ( [float(f) for f in ret.subset.item[time_step].strip().split(",")[5:]])
